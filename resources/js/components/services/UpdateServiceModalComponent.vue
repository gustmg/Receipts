    ...
    <div id="updateServiceModal" class="modal updateServiceModal modal-fixed-footer">
        <div class="modal-content">
            <div class="row no-margin-bottom">
                <div class="col m12">
                    <h6><b>Editar Services</b></h6>
                </div>
            </div>
            <FormulateForm class="row" name="updateServiceForm" v-model="updateServiceForm">
                <input type="hidden" name="_token" :value="csrf" />
                <FormulateInput type="hidden" name="service_id" v-model="serviceToUpdate.service_id" />
                ...
                <FormulateInput
                    ...
                    v-on:blur="validateSalePrice(serviceToUpdate.service_cost, serviceToUpdate.prev_cost)"
                    v-currency
                />
        ...
        <div class="modal-footer">
            <button v-on:click="resetUpdateServiceForm" class="modal-action modal-close waves-effect btn-flat">
                <b>Cancelar</b>
            </button>
            <button
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('services', ['serviceToUpdate', 'serviceToUpdatePrevCost']),

            validateForm: function(e) {
                if (
                    this.invalidServiceName ||
                    this.invalidServiceCost ||
                    this.invalidServiceBasePricePercentage ||
                    this.invalidServiceRetailPricePercentage ||
                    this.invalidServiceWholesalePricePercentage
                ) {
                    return true
                } else {
                    return false
