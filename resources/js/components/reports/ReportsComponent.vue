            ...
            <table class="centered highlight">
                ...
                <tbody>
                    <tr v-for="sale in sales" v-bind:key="sale.sale_id">
                        ...
                        <td><a class="modal-trigger" href="#sale-detail-modal" v-on:click="setSaleDetailData(sale)"><i class="material-icons">open_in_new</i></a></td>
                    </tr>
                </tbody>
            </table>
        </div>
                            ...
                            <table class="card centered">
                                ...
                                <tbody>
                                    <tr v-for="service in sale_detail_services" v-bind:key="service.service_id">
                                        ...
                                        <td class="right-align">${{(service.pivot.service_quantity * service.pivot.service_unit_price).toFixed(2)}}</td>
                                    </tr>
                                    <tr v-for="product in sale_detail_products" v-bind:key="product.product_id">
                                        ...
                                        <td class="right-align">${{(product.pivot.product_quantity * product.pivot.product_unit_price).toFixed(2)}}</td>
                                    </tr>
                                </tbody>
                            </table>
                            <div class="container-fluid">
                                <div class="row no-margin">
                                    ...
                                    <div class="col m2" align="right">
                                        <span class="padding-r-20"><b>${{sale_detail_subtotal}}</b></span><br>
                                        <span class="padding-r-20"><b>${{sale_detail_vat}}</b></span><br>
                                        <span class="padding-r-20"><b>${{sale_detail_credit_card_comision}}</b></span><br>
                                        <span class="padding-r-20"><b>${{sale_detail_total}}</b></span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            sale_detail_vat: function() {
                ...
                if(this.sale_detail_invoiced){
                    if(this.sale_detail_payment_form_id==2 && this.sale_detail_total > 999){
                        vat = (this.sale_detail_total - this.sale_detail_credit_card_comision) - ((this.sale_detail_total - this.sale_detail_credit_card_comision) / 1.16);
                    }
                    else{
